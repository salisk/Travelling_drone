...
def calculateMove(gameState):
    ...
    best_order = a
    best_distance = route_distance(a, gameState)
        # persistentData["i"] = 0
        # persistentData["j"] = 0
    # else:
    ...
    while improve:
        ...
        for i in range(len(best_order) - 1):
            ...
            for j in range(i + 1, len(best_order)):
                #j + persistentData["j"]
                c =  choice( list( range(j+1, len(best_order)) ) )
                a, b, c = choice( list( permutations( (a, b, c) ) ) )
                new_order = best_order[:a] + best_order[a:b+1][::-1] + best_order[b:c+1][::-1] + best_order[c:]
